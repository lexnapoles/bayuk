const isAsyncActionType = (action, { request, success }) =>
  action[request] && action[success];

const findType = (action, key, type) => type[key] === action.type;

const defaultKeys = { request: "request", success: "success" };

const isFetching = (actions, keys = defaultKeys) => (state = true, action) => {
  if (!Array.isArray(actions)) {
    actions = [actions];
  }

  const someActionIsNotAsync = actions.some(
    anAction => !isAsyncActionType(anAction)
  );
