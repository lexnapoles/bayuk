...
export const getImagePath = (id, dir) =>
  `${path.join(process.env.IMAGESDIR, `/${dir}`, id)}.jpg`;

export const getDecodedImage = data => {
  const img = data.replace(/^data:image\/\w+;base64,/, "");

  return Buffer.alloc(img.length, img, "base64");
};

export const isImageObjValid = image => has(image, "id") && has(image, "data");

const writeOneImageToDisk = ({ path: imagePath, data }) =>
...
export const getImagesToDelete = (newImages, oldImages) => {
  const unmodifiedImages = extractIdsFromImageUrls(
    newImages.filter(img => !isImageBase64(img))
  );
